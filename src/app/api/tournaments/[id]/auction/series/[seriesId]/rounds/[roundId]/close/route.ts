...
import { closeAuctionRound } from "@/services/server/auction.service";
...
export async function POST(_req: Request, { params }: Params) {
  ...
  if (!isSystemAdmin) {
    return NextResponse.json(
      { success: false, error: { code: "FORBIDDEN", message: "Only admins can close bidding" } },
      { status: 403 },
    );
  }

  const result = await closeAuctionRound(id, seriesId, roundId);
